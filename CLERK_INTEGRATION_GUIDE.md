# Integrate Clerk into an Existing React (Vite) App
**Purpose:** Enforce only the **current** and **correct** instructions for integrating [Clerk](https://clerk.com/) into an **existing** React (Vite) application.  
**Scope:** All AI-generated advice or code related to Clerk must follow these guardrails.

## **1. OFFICIAL CLERK + REACT (VITE) SETUP (Existing Project)**
1. Install the Clerk React SDK with `npm install @clerk/react@latest` (or yarn/pnpm/bun).
2. Set `VITE_CLERK_PUBLISHABLE_KEY` in `.env.local` or `.env`.  
   **Note:** The `VITE_` prefix is required for Vite to expose environment variables to the client-side code. `.env.local` is preferred for local development secrets.
3. Wrap the app in `<ClerkProvider>` within `main.tsx` or `main.jsx`.
4. Use Clerk's `<Show>`, `<SignInButton>`, `<SignUpButton>`, `<UserButton>` in the app.

### **Correct, Up-to-Date Integration Code Examples (for existing projects)**

1. **Navigate to your existing React + Vite project** (TypeScript example)
   ```bash
   cd your-existing-react-vite-app
   ```
...
When asked about integrating Clerk into an **existing** React (Vite) app, your response **MUST**:
